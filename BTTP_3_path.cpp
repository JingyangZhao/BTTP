...
void normalSupergameTriangle(int above, int below, vector<int> &x, vector<int> &y, vector<vector<int> > &table, int d, int f)
{
	...
	for(int i=1; i<=3; i++){
		for(int j=0; j<3; j++){
			table[X[j]][d] = f*Y[(j+i)%3];
			table[Y[(j+i)%3]][d] = -f*X[j];
		}
		d++;
	}
	f = -f;
...
double construction(int n, vector<int> &x, vector<int> &y, vector<vector<double> > &newmatrix, vector<vector<int> > &table, int direction)
{
	...
	for(int i=1; i<=m; i++)
	{
		...
		for(int j=1; j<=m; j++)
		{
			...
			if(j==1)
			{
				leftSupergame(n, above, below, x, y, table, 6*i-4, direction);
			}
			else
			{
				normalSupergame(above, below, x, y, table, 6*i-4, direction);
			}
		}
	}
